...
#------------------------------
    ...
    clawdata.lower[0] = -124.65
    clawdata.upper[0] = -123.65
    clawdata.lower[1] = 46.8
    clawdata.upper[1] = 47.1

    clawdata.num_cells[0] = 60
    clawdata.num_cells[1] = 18
    ...
    if clawdata.output_style==1:
        ...
        clawdata.num_output_times = 3*24
        clawdata.tfinal = 3*24*3600.
    ...
    # 1', 12", 4", 1", 1/3"
    amrdata.refinement_ratios_x = [5,3,4,3]
    amrdata.refinement_ratios_y = [5,3,4,3]
    amrdata.refinement_ratios_t = [5,3,4,3]
    ...
    regions.append([1,1,0.,1e9,clawdata.lower[0]-0.1,clawdata.upper[0]+0.1,
                    clawdata.lower[1]-0.1,clawdata.upper[1]+0.1])
    regions.append([1,2,0.,1e9,-124.25, -123.65, 46.8, 47.1])
    regions.append([1,3,0.,1e9,-124.2, -123.65, 46.8, 47.05])
    regions.append([2,4,0.,1e9,-124.119,-124.097,46.903, 46.915]) # Wp marina
    regions.append([2,4,0.,1e9,-123.89,-123.73,46.94, 46.996])  # Aberdeen
    ...
    gauges.append([0,-124.4,46.92,0,1e9])  # Offshore tide
    gauges.append([1,-124.11,46.92,0,1e9])  # GH inlet
    gauges.append([2,-123.92,46.95,0,1e9])
    gauges.append([3,-123.85,46.96,0,1e9])
    gauges.append([4,-124.00,46.90,0,1e9])  # Johns River
    gauges.append([5,-123.808,46.972,0,1e9])  # Chehalis at Wishkah

    gauges.append([1187,-123.8571, 46.9657, 0,1e9])  # Aberdeen
    gauges.append([1102,-124.10485, 46.90447, 0,1e9])    # Westport

    gauges.append([8,-124.65,46.96,0,1e9])  # at Left Boundary
...
#-------------------
    ...
    refinement_data.speed_tolerance = 6*[0.05]
    ...
    topofiles.append([3, topodir + 'etopo1_-125_-123_46_48_1min.asc'])
    topofiles.append([3, topodir+'GH_13sec_mtl.asc'])
